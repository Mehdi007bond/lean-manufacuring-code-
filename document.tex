...
\begin{itemize}
\item \textbf{Just-In-Time (JIT) :} Produire exactement ce qui est
\end{itemize}
nécessaire, quand c\textquotesingle est nécessaire et en quantité
voulue. Repose sur le flux tiré (\textbf{Kanban}).

\begin{itemize}
\item \textbf{Kaizen :} Démarche d\textquotesingle amélioration continue par
\end{itemize}
"petits pas", impliquant tout le personnel, du terrain à la direction.

\begin{itemize}
\item \textbf{VSM (Value Stream Mapping) :} Cartographie du flux de valeur
\end{itemize}
permettant de visualiser les gaspillages entre le fournisseur et le
client.
...
\begin{itemize}
\item \textbf{Situation :} Le standard existe, il était maîtrisé, mais
\end{itemize}
"quelque chose" a cassé le processus.

\begin{itemize}
\item \textbf{L\textquotesingle objectif :} C\textquotesingle est un retour
\end{itemize}
vers le passé (revenir à l\textquotesingle état stable que
l\textquotesingle on connaissait).

\begin{itemize}
\item \textbf{Le mot-clé :Réactivité}. On parle de "Correctif".

\item \textbf{Analogie :} Un athlète qui court habituellement le 100m en 10
\end{itemize}
secondes (objectif atteint) et
...
\begin{itemize}
\item \textbf{Situation :} Le processus est stable, il n\textquotesingle y a
\end{itemize}
pas de "panne", mais le niveau actuel ne suffit plus

pour être compétitif.

\begin{itemize}
\item \textbf{L\textquotesingle objectif :} C\textquotesingle est un bond
\end{itemize}
vers le futur (atteindre un niveau de performance jamais atteint

auparavant).

\begin{itemize}
\item \textbf{Le mot-clé :Proactif}. On parle d\textquotesingle{}
\end{itemize}
Amélioration de rupture (Breakthrough).

\begin{itemize}
\item \textbf{Analogie :} Le même athlète court toujours en 10 secondes
\end{itemize}
(stable), mais pour gagner les JO,
...
\begin{itemize}
\item Mécontentement des donneurs d'ordres : retards de livraison et coût
\end{itemize}
élevé des défauts qualité.

\begin{itemize}
\item Menace de changement de fournisseur sous 6 mois si la situation n'est
\end{itemize}
pas redressée.

\begin{itemize}
\item Performance technique : 45 pannes/mois, temps de change ment de
\end{itemize}
référence de 3h, délai de livraison moyen de 7 jours.
\end{minipage} \\
\begin{minipage}[t]{\linewidth}\raggedright
Situation future
\end{minipage} & \begin{tabular}{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 2\tabcolsep) * \real{0.5000}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 2\tabcolsep) * \real{0.5000}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
...
\end{itemize}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Clients satisfaits et fidélisés grâce à une meilleure fiabilité.

Délais de livraison réduits de 20\% (cible : 5,6 jours).

Entreprise rentable avec une marge supérieure à 20\%.

Processus stabilisé avec une réduction significative des pannes et du temps SMED.
\end{minipage} \\
\midrule\noalign{}
...
\end{minipage} & \begin{minipage}[t]{\linewidth}\raggedright
\begin{itemize}
\item Perte définitive des clients majeurs si le délai de 6 mois n'est pas
\end{itemize}
respecté.

\begin{itemize}
\item Instabilité critique de la production due au nombre élevé de pannes
\end{itemize}
(45/mois).
...
\item Suivi mensuel des indicateurs clés (KPI) de performance pour les
\end{itemize}
donneurs
...
\begin{minipage}[b]{\linewidth}\raggedright
\begin{itemize}
\item Plan de communication :\\
\end{itemize}
\textbf{Mesurer} Taux de client
...
\begin{itemize}
\item \textbf{Concentration des Produits :} Les \textbf{5 premiers produits}
\end{itemize}
(sur 10) génèrent \textbf{67.66\%} du Chiffre d\textquotesingle Affaires
total.
...
\begin{itemize}
\item \textbf{Concentration des Clients :} Étant donné qu\textquotesingle il
\end{itemize}
n\textquotesingle y a que 5 clients uniques, le top 5 des clients génère
...
\begin{itemize}
\item \textbf{Gestion des Stocks :} Pour \textbf{P02} et \textbf{P10}, une
\end{itemize}
rupture de stock serait catastrophique. Ils nécessitent un suivi
...
\end{itemize}
de la Production :} Il est probable que vos lignes de production soient
...
\begin{itemize}
\item \textbf{Analyse de Rentabilité :} Il serait intéressant de vérifier si
\end{itemize}
ces deux produits à fort volume sont aussi ceux qui génèrent le plus de
...
\begin{itemize}
\item \textbf{Risque Client/Produit :} Notre rentabilité est mieux répartie
\end{itemize}
que votre production. Si la demande pour P02 chute,
...
\begin{itemize}
\item \textbf{Analyse du Prix Unitaire :} On constate un écart massif. Par
\end{itemize}
exemple, le produit \textbf{P01} (2,7 millions de CA pour seulement 107
